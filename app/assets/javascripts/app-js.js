...
var _network = __webpack_require__(329);
...
function drawGraph(orgName) {
  ...
  var color = d3.scale.linear().domain([min_score, (min_score + max_score) / 2, max_score]).range(["lime", "yellow", "#28a745"]);
  ...
  var force = d3.layout.force().linkDistance(200).charge(-500).size([w, h]);
  ...
  d3.json(endpoint, function (error, graph) {
    ...
    var link = g.selectAll(".link").data(graph.links).enter().append("line").attr("class", "link").style("stroke-width", nominal_stroke).style("stroke", function (d) {
    ...
    var circle = node.append("path").attr("d", d3.svg.symbol().size(function (d) {
    ...
    var text = g.selectAll(".text").data(graph.nodes).enter().append("text").attr("dy", ".35em").attr("fill", "#bbff99").style("font-size", nominal_text_size + "px");
    ...
    function keydown() {
      if (_d.event.keyCode == 32) {
        force.stop();
      } else if (_d.event.keyCode >= 48 && _d.event.keyCode <= 90 && !_d.event.ctrlKey && !_d.event.altKey && !_d.event.metaKey) {
        switch (String.fromCharCode(_d.event.keyCode)) {
          case "C":
            keyc = !keyc;break;
          case "S":
            keys = !keys;break;
